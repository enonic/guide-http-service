...
****
This guide will provide you the basic knowledge about writing *Http services* in Enonic XP.
****
...
In order to create a service, you’ll need to create a js controller and place it under `src/main/resources/services/<service-name>/<service-name>.js`

NOTE: JS controllers should return the following https://developer.enonic.com/docs/xp/stable/framework/http#http-response[response] object.
...
The url you choose to call your service will *define the context of the call*. For example, depending on the chosen url, you'll get your service running on different projects:
...
NOTE: https://developer.enonic.com/docs/xp/stable/api/lib-context[lib context] can be used inside the service controller to ensure a specific context on the call.

== Access restriction
It is also possible to restrict access to the service based on user roles. 

This can be done without custom coding, and you just need to add a `<service-name>.xml` file in your service folder with the following structure
...
Instead of manually writing a valid service url to call it in your application, Enonic portal JS api provides a method https://developer.enonic.com/docs/xp/stable/api/lib-portal#serviceurl[serviceUrl] to generate a valid url pointing to a service.

NOTE: The `serviceUrl`` function will mount a url to a service based on the context of where it was called.

== Example of a service
Now that we know what services are, lets come up with our own.
...
. Since we’d like to fetch the person content type, it makes sense to call our service something like people or getPeople. Let’s stick with *people*, therefore create a js file: `src/main/resources/services/people/people.js` with the following code:
+
[source,Javascript]
----
...
.. `s` will be a string parameter to look for person *firstName or lastName*
.. `l` will be a parameter to *limit the number of results*
.. `s` default will be an empty string and `l` default will be 5. 

. Now that we've defined that we’ll use parameters, let’s investigate what is returned on the request parameter of the function that get exposed as *GET*. To do that, update the service code to:
+
[source,Javascript]
----
exports.get = function (request) {
...
+
NOTE: The *project name* can be found on the top left corner of content studio. If you haven't setup a new project, it is probably `default`. *Branch* will be `draft` or `master`. *App name* was already described before, and is located in `gradle.properties`

NOTE: Note that depending on the context it is possible to get no data as a return. The reason for that is probably because in that context you don't have person content type instances to be queried by content lib.

== Other common use cases
In the previous example we wrote service that provided content type data based on query parameters.
...
- Much more creative scnearios when used together with some https://developer.enonic.com/docs/xp/stable/api[Enonic JS libraries]